...
: >"${env_file}"
chmod 600 "${env_file}"
passthrough='^(PATH|TZ|TZDIR|LANG|LOCALE_ARCHIVE|TERMINFO_DIRS|HOME|TMPDIR|SSL_CERT_FILE|NIX_SSL_CERT_FILE|GIT_SSL_CAINFO|KUBERNETES_SERVICE_HOST|KUBERNETES_SERVICE_PORT|KUBERNETES_SERVICE_PORT_HTTPS|TALOSCONFIG|GH_TOKEN|GIT_USER_NAME|GIT_USER_EMAIL|MOSH_PORT_RANGE|DISABLE_AUTOUPDATER|USE_BUILTIN_RIPGREP)$'
passthrough+='|^(PI|BASTION|TELEGRAM|AWS|AZURE_OPENAI|CLOUDFLARE|OLLAMA|OPENAI|ANTHROPIC|CLAUDE|GOOGLE|VERTEX)_'
passthrough+='|_API_KEY$|_TOKEN$|_BASE_URL$'
while IFS= read -r -d '' entry; do
  var="${entry%%=*}"
  if [[ "${var}" =~ ${passthrough} && -n "${entry#*=}" ]]; then
    printf '%s=%s\n' "${var}" "${entry#*=}" >>"${env_file}"
  fi
done < <(env -0)
...
if [[ "${BASTION_AGENT_ENABLED:-true}" == "true" ]]; then
  tmux new-session -d -s pi -c "${HOME}" pi-channel
fi
